...
fn count_score(board: &Board, side: Side, king_multiplier: f64) -> f64 {
    match side {
        Side::Black => {
            let our_checkers_amount = board.count(Cell::Black) as f64;
            let our_kings_amount = board.count(Cell::BlackKing) as f64;
            let enemy_checkers_amount = board.count(Cell::White) as f64;
            let enemy_kings_amount = board.count(Cell::WhiteKing) as f64;
            return (our_checkers_amount + our_kings_amount * king_multiplier) - (enemy_checkers_amount + enemy_kings_amount * king_multiplier);
        },
        Side::White => {
            let our_checkers_amount = board.count(Cell::White) as f64;
            let our_kings_amount = board.count(Cell::WhiteKing) as f64;
            let enemy_checkers_amount = board.count(Cell::Black) as f64;
            let enemy_kings_amount = board.count(Cell::BlackKing) as f64;
            return (our_checkers_amount + our_kings_amount * king_multiplier) - (enemy_checkers_amount + enemy_kings_amount * king_multiplier);
        },
    }
}
...
fn moves_with_scores(board: &Board, depth: usize, king_multiplier: f64) -> Vec<ScoredMove> {
    ...
    for i in 0..mvs.len() {
        ...
        let score = match test_board.who_win() {
            Some(x) if x == board.who_turn() => 9999.,
            Some(x) if x != board.who_turn() => -9999.,
            None => {
                match depth {
                    0 => count_score(&test_board, board.who_turn(), king_multiplier),
                    _ => {
                        let enemy_moves = moves_with_scores(&test_board, depth - 1, king_multiplier);
                        let mut best_enemy_move_score = -999.;
...
fn best_move(board: &Board, depth: usize, king_multiplier: f64) -> Option<Move> {
